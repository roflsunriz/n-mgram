...
デスクトップでは右綴じ見開き、Androidの縦持ちでは1ページ表示を基本にし、左右スワイプと画面の左右タップでページを送れます。縦読みでは左右スワイプで前後の章へ移動でき、先頭画像を下へ引くと画像を再読み込みします。ピンチとダブルタップで1〜4倍に拡大し、拡大中はマウスまたは1本指のドラッグで表示位置を移動できます。Windows版ではツールバー、タッチパッドのピンチ、`Ctrl++`・`Ctrl+-`・`Ctrl+0`からも倍率を操作できます。小さい画面では主要4ページを下部ナビゲーションへ置き、端末のセーフエリアも考慮します。
