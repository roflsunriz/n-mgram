...
describe('chapter edge prefetch', () => {
  it('selects the first two pages of the oldest and newest chapters', () => {
    const chapters = [
    ...
    expect(getChapterEdgePageUrls(chapters)).toEqual(['1-1', '1-2', '8-1', '8-2']);
  });
  ...
  it('deduplicates a single chapter and releases every prefetched cache reference', async () => {
    ...
    expect(acquirePageImage).toHaveBeenCalledTimes(2);
